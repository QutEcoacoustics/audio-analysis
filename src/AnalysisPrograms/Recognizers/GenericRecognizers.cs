...
// As of January 2020, this syllable recogniser employs the following steps:
...
//
// As of January 2020, this oscilolation syllable recogniser employs the following steps:
// 1 - 3. Same as above.
...
namespace AnalysisPrograms.Recognizers
{
    ...
    /// <summary>
    /// This is a recognizer for generic syllable types.
    /// </summary>
    internal class GenericRecognizers : RecognizerBase
    {
        ...
        */
        ...
        public override RecognizerResults Recognize(AudioRecording audioRecording, Config genericConfig, TimeSpan segmentStartOffset, Lazy<IndexCalculateResult[]> getSpectralIndexes, DirectoryInfo outputDirectory, int? imageWidth)
        {
            ...
            {
                GrLog.Warn("No configuration profiles found. Two profiles expected for the Flying Fox recogniser.");
            }
            ...
            if (ConfigFile.TryGetProfile(genericConfig, "Territorial", out var _))
            {
                ...
                GrLog.Debug("Territory event count = " + territorialResults.Events.Count);
            }
            else
            {
                GrLog.Warn("Could not access Territorial configuration parameters");
            }
            ...
            if (ConfigFile.TryGetProfile(genericConfig, "Wingbeats", out var _))
            {
                ...
                GrLog.Debug("Wingbeat event count = " + wingbeatResults.Events.Count);
            }
            else
            {
                GrLog.Warn("Could not access Wingbeats configuration parameters");
            }
