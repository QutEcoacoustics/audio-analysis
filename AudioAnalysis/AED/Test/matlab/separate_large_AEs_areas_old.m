function [newAE] = separate_large_AEs_areas_old(oldAE,L,area_thresh,I1)
...
%     figure(32), clf, imshow(finalI2)
    ...
    if maxL>0
        for kk=1:maxL
            ...
            thisAE(5) = startN - 1 + thisAE(1) + (thisAE(3)-1)/2; %col centroid
            thisAE(6) = startM - 1 + thisAE(2) + (thisAE(4)-1)/2; %row centroid
            newAE = [newAE thisAE];
        end
    end
...
%     figure(33), clf, imshow(finalI1)
    ...
    if maxL>0
        for kk=1:maxL
            ...
            thisAE(3) = max(tmpy)-min(tmpy)+1; %height
            thisAE(4) = tM; % width
            thisAE(5) = thisAE(1) + (thisAE(3)-1)/2; %col centroid
            thisAE(6) = thisAE(2) + (thisAE(4)-1)/2; %row centroid
            newAE = [newAE thisAE];
        end
    end
