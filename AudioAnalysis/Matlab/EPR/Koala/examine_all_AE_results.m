...
for ng=1:num_g
    ...
    if issame(this_results_list(ng,1:minlen),name(1:minlen))
        ...
        if num_d > 0
            ...
            [tmp, thisAE_left_ind] = min(thisAE(:,1));

            score =  all_results(ng,8);
            if score < match_score
                notAE = [notAE; thisAE(thisAE_left_ind,:)];
            else
                isAE = [isAE; thisAE(thisAE_left_ind,:)];
            end
        end
    end
end
...
show_image(I2,T,F,tmax,fmax,7,allAE',name)
show_image(I2,T,F,tmax,fmax,7,notAE',name,2)
show_image(I2,T,F,tmax,fmax,7,isAE',name,1)
