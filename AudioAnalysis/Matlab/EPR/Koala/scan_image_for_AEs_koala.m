function scan_image_for_AEs_koala(name, file_path_audio, file_path_acoustic_events, results_path, int_thresh, small_events_thresh, xlsfile, template_name)
% Get all AEs where the min freq is within a specific freq band (fband_min,fband_max) 
%
% THIS FUNCTION IS SPECIFICALLY FOR THE KOALA BELLOW
%
...
%[this_results] = xlsread(strcat(name,'_Intensity_Thresh_',num2str(int_thresh),'dB.xls'));
cd(working_path)
...
thresh = 500;
freq_bottom_thresh1 = template_bottom - thresh; % HARDCODED 
freq_bottom_thresh1(freq_bottom_thresh1<0) = 0;
freq_bottom_thresh2 = template_bottom + thresh; % HARDCODED 
...
freq_top = allAE(:,4);
freq_bottom = allAE(:,3);
freq_bottom_centroid = freq_bottom + (freq_top-freq_bottom)/2;
indAE = find( (freq_bottom_centroid > freq_bottom_thresh1) & (freq_bottom_centroid < freq_bottom_thresh2) );
lenAE = length(indAE);
AE = allAE(indAE,:);
...
% retrieve image (size of template) with AE on left hand side
for na = 1:lenAE
    ...
    this_freq_bottom = template_bottom; % use template, not first AE to specify this
    [tmp2, bottom] = min(abs(F-this_freq_bottom));
    this_freq_top = template_top; % use template, not first AE to specify this
    [tmp2, top] = min(abs(F-this_freq_top));
%     I3_seg_all = zeros(tM,tN);
%     I3_seg_all(1:top-bottom+1,1:right-left+1) = I3(bottom:top,left:right);
    
    
    % find all acoustic events that fit within these boundaries (partially)
%     ind_boundary_AE = find( ( ((this_time_start<time_end) & (this_time_end>=time_end))  | ((this_time_start<=time_start) & (this_time_end>time_start)) ) & ( ((this_freq_bottom<freq_top) & (this_freq_top>=freq_top)) | ((this_freq_bottom<=freq_bottom) & (this_freq_top>freq_bottom)) ) );
    ind_boundary_AE = find( (this_time_start<=time_start) & (this_time_end>time_start) & ( ((this_freq_bottom<freq_top) & (this_freq_top>=freq_top)) | ((this_freq_bottom<=freq_bottom) & (this_freq_top>freq_bottom)) ) );
    
    
    num_boundary_AE = length(ind_boundary_AE);
    boundary_AE = allAE(ind_boundary_AE,:);

%     show_image(I2,T,F,tmax,fmax,14,AE(na,:)')
%     show_image(I2,T,F,tmax,fmax,14,boundary_AE',1)
%     pause
