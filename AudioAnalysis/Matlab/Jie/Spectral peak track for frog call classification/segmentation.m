...
audioFolder = 'C:\work\dataset\audio data\MMSP\*.wav';
audioFolderName = 'C:\work\dataset\audio data\MMSP\fileName';
  
audioList = dir(audioFolder);
% remove hidden files
audioList = audioList(arrayfun(@(x) ~strcmp(x.name(1), '.'), audioList));

% smoothIndex = [6, 18, 20, 22, 23];
...
%     iAudio = smoothIndex(index);
    ...
    path = '.\05-01\segmentation\fileName.csv';
    frog = frogName.name;
