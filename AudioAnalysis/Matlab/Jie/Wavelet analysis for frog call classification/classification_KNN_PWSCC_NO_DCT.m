...
pathPWSCC = 'C:\Users\Jie\Documents\MATLAB\Tasks\E-science conference\07-22\PWSCC_NO_DCT';
featureFolderName = 'C:\Users\Jie\Documents\MATLAB\Tasks\E-science conference\07-22\PWSCC_NO_DCT\fileName';
PWSCCList = dir(pathPWSCC);
PWSCCList = PWSCCList(arrayfun(@(x) ~strcmp(x.name(1), '.'), PWSCCList));

featureResult = zeros(1, 15);
for iSpecies = 1 : length(PWSCCList)
    ...
    fid = fopen(frogPath);
    out = textscan(fid, '%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s', 'delimiter', ',');
    fclose(fid);
    
    % put features of different frog species together
    len = length(out{1});
    frogFeature = zeros(len - 1, 15);
...
feature = featureResult(:, 1:14);
label = featureResult(:, 15);
...
csvwrite('.\07-10\classification\WPDEnergy.csv', meanResult);
