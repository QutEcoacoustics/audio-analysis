...
namespace TowseyLib
{
    ...
    public class MarkovModel
    {
        ...
        public void ScoreSequence_v2(int[] intSequence, out double[] scores, out int hitCount, out double bestHit, out int bestFrame)
        {
            ...
            int L = intSequence.Length;
            int garbageID = this.numberOfStates - 1;
            if (this.GraphType == HMMType.TWO_STATE_PERIODIC) garbageID = 2;
            ...
            for (int i = 1; i < L - extractLength; i++) //parse sequence
            {
                ...
                int count;
                if (this.GraphType == HMMType.TWO_STATE_PERIODIC)
        ...
        public static HMMType GetHmmType(string name)
        {
            try
            {
                return (HMMType)Enum.Parse(typeof(HMMType), name);
            }
			catch
            {
                return HMMType.UNDEFINED;
            }
        }
