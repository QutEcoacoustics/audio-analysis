...
#   occurrence <- count.occurrence(species)
  rules <- apriori(species, parameter=list(minlen=2, supp=0.001, conf=0.6, maxlen=2), 
...
#     rule.count <- rule.count + length(which(chiSquared>3.84))
    ...
    filepath <- paste('c:/work/myfile/daytime_supp0.01conf0.5_training/', filedate, '.csv', sep="")
