...
#todo
Sp.createTargeted <- function (site, start.date, start.sec, duration, img.path = NA, rects = NA) {
    ...
    # 2. identify whether the target is within 1 audio file or more than one
    # 3. idenify the sample number for the start and the sample number for the end
    ...
    for (i in 1:nrow(file.positions)) {   
        ...
        if (is.na(samp.rate) && is.na(bit)) {
            ...
            stop('requested segment spans multiple files of different sample rates and/or bit resolutions');    
        }
    ...
    w <- Wave(left = wav.samples, right = numeric(0), samp.rate = samp.rate, bit = bit)
    spec <- Sp.create(w);
    spec$rects <- rects;
    if (!is.na(img.path)) {
...
Sp.create <- function(wav, frame.width = 512, draw = FALSE, smooth = TRUE, db = TRUE, filename = FALSE) {
...
#      draw: boolean; whether to draw the spectrogram or just return the values
#      smooth: boolean; whether to perform smoothing
#      db: boolean; whether to convert to db (log). without this, low values will be imperceptible
#      filename: string or FALSE; the path where to save the image file, or FALSE to not save the spectrogram
#
#  Returns:
#      spectrogram; (custom object) each column is a time-frame, each row is a frequency bin \n
  ...
  #wav can be a string or a tuneR wav object. If it's a string then create a tuneR wav object
...
#    bit.resolution: Int; How many bits is the amplitude resolution of the wave
#
# Returns: 
#     matrix
#
...
Sp.draw <- function(spectro, img.path = FALSE) {
    ...
    #    rects: matrix of start.sec, duration, top freq, bottom freq, color, row for each rectangle
    #
    #  Returns:
    #    NULL
    require('grid');
...
#    frames.per.sec = (ncol(amp))/(len/samp.rate))
    ...
    height <- (rect.borders$top.freq - rect.borders$bottom.freq) / spectro$frequency.range;
    
    if (!is.null(rect.borders$rect.color)) {
        rect.col <- rect.borders$rect.color;
    } else {
    ...
    grid.rect(x = unit(x, "npc"), 
              ...
              gp=gpar(col = "white", fill=rect.col, alpha=0.3), draw = TRUE, vp = NULL) 
