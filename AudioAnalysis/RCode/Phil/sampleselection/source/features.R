...
Read.Events <- function () {
  return(read.csv(OutputPath('events'), header = FALSE, col.names = g.events.col.names));
}
...
DoFeatureExtraction <- function () {
  ...
  features.all = as.data.frame(matrix(data = features.all, nrow = nrow(events), byrow = TRUE));
  # have not escaped separator char, but shouldn't matter with numeric values anyway
  write.table(features.all, file = OutputPath('features'), append = FALSE, col.names = TRUE, row.names = FALSE, sep = ",", quote = FALSE);
...
GetFeatures <- function (bounds, spectro) {
  ...
  center.freqs <- CenterFreqs(event_vals, spectro$hz.per.bin, features$bottom.f);
  ...
  return(c(features, list(
    ...
    # i.e. how much the standard deviation of frequency changes during the event
...
CenterFreqs <- function (m, hz.per.bin, low.f) {
    ...
    # this indicates how much in general the frequency of the event is increasing or decreasing from start to finish
    # note that both a flat line and a line which increases then decreases could end up with a similar slope, because it is fitting a straight line
    #
    # Args:
    ...
    # Returns:
    #    list: with 2 values:
    #            cfs.slope: the slope of the center frequency calculated for each frame
    #            cfs.mean: the center frequency calculated on the mean db value for each frequency bin
    #                      (this is not the same as the mean center frequency after calculating for each time frame)
...
BbScores <- function (m) {
    ...
    bbscores <- apply(m, 2, function (col) {
