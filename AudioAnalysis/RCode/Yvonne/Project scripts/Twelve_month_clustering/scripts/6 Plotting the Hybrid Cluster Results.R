...
plot_2hour_files <- function(clust_num, colour) {
  if(clust_num < 10) {
    png(filename = paste("data/2hour_plots_", k1_value, "_",
                         k2_value,"/2hourPlot_Cluster_colour",
                         "0", clust_num,"_", k1_value, "_", k2_value, 
                         ".png", sep = ""), 
        width = 1600, 
        height = 500, 
  ...
  max <- max(cluster_reference$output)
  par(mfrow=c(2,ceiling(num_of_plots/2)), 
      mar=c(1,1,5,0), oma=c(2,5,2.5,2), xpd = NA,
      cex.axis=1.6)
  for(i in 1:num_of_plots) { # num_of_plots should be even
    ...
    if(i %in% num1) {
      ...
      Axis(side = 1, labels = FALSE, 
           at = seq.int(0.8, 14, length.out = 12))
    }
    num2 <- c(2:14,16:28) 
    if(i %in% num2) {
      barplot(cluster_reference$output[started[i]:finished[i]],
      ...
      Axis(side = 1, labels = FALSE, at = seq.int(0.8, 14, length.out = 12))
    }
    mtext(side = 3, paste(months[i]), line = 0.3, cex = 1.6)
    mtext(side = 1, text = c(as.character(seq(0,24,12))), 
          at = c(0,7,14), line=0.8, cex = 1.3)
  }
  mtext(side = 3, line = -0.5, paste("Cluster ", clust_num,
                                 "  -  (k1 = ",k1_value,", k2 = ", k2_value,")", 
                                 sep=""), cex=2, outer = T)
  mtext(side = 3, line = -2.9, adj = 0,
        "     Gympie NP", cex= 1.2, outer = T)
  mtext(side = 3, line = -27, adj = 0,
        "     Woondum NP", cex= 1.2, outer = T)
  mtext(side = 2, line = 3,
        "Average number of minutes in 2 hour period", 
        cex = 1.6, outer = T)
...
# grey, orange, lightblue, green, yellow, darkblue, burntorange, purplepink
...
cluster <- 48
for(i in cluster:cluster) {     #k2_value) {
  plot_2hour_files(i, colour = cicadas)  
}
...
# Choose either ordinary colours (greater range) or
# colourblind colours (limited range) for the plot
...
if(version=="colourblind") {
  dom_agg_cl_gym_colblind <- dom_agg_cl_gym
  dom_agg_cl_gym_colblind$feature <- as.character(dom_agg_cl_gym_colblind$feature)
  a <- which(dom_agg_cl_gym_colblind$feature=="RAIN LIGHT")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("RAIN")
  a <- which(dom_agg_cl_gym_colblind$feature=="QUIET FAIRLY")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("QUIET")
  a <- which(dom_agg_cl_gym_colblind$feature=="QUIET MOSTLY")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("QUIET")
  a <- which(dom_agg_cl_gym_colblind$feature=="WIND MODERATE")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("WIND")
  a <- which(dom_agg_cl_gym_colblind$feature=="RAIN MODERATE")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("RAIN")
  a <- which(dom_agg_cl_gym_colblind$feature=="QUIET INSECTS")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("QUIET")
  a <- which(dom_agg_cl_gym_colblind$feature=="LOW FREQUENCY")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("PLANES")
  a <- which(dom_agg_cl_gym_colblind$feature=="WIND STRONG")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("WIND")
  a <- which(dom_agg_cl_gym_colblind$feature=="WIND SLIGHT")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("WIND")
  a <- which(dom_agg_cl_gym_colblind$feature=="MORNING CHORUS")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("BIRDS")
  a <- which(dom_agg_cl_gym_colblind$feature=="WIND VERY STRONG")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("WIND")
  a <- which(dom_agg_cl_gym_colblind$feature=="BIRDS LOUD")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("BIRDS")
  a <- which(dom_agg_cl_gym_colblind$feature=="RAIN HEAVY")
  dom_agg_cl_gym_colblind$feature[a] <- as.character("RAIN")
  dom_agg_cl_gym <- dom_agg_cl_gym_colblind
  dom_agg_cl_woon_colblind <- dom_agg_cl_woon
  dom_agg_cl_woon_colblind$feature <- as.character(dom_agg_cl_woon_colblind$feature)
  a <- which(dom_agg_cl_woon_colblind$feature=="RAIN LIGHT")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("RAIN")
  a <- which(dom_agg_cl_woon_colblind$feature=="QUIET FAIRLY")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("QUIET")
  a <- which(dom_agg_cl_woon_colblind$feature=="QUIET MOSTLY")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("QUIET")
  a <- which(dom_agg_cl_woon_colblind$feature=="WIND MODERATE")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("WIND")
  a <- which(dom_agg_cl_woon_colblind$feature=="RAIN MODERATE")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("RAIN")
  a <- which(dom_agg_cl_woon_colblind$feature=="QUIET INSECTS")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("QUIET")
  a <- which(dom_agg_cl_woon_colblind$feature=="LOW FREQUENCY")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("PLANES")
  a <- which(dom_agg_cl_woon_colblind$feature=="WIND STRONG")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("WIND")
  a <- which(dom_agg_cl_woon_colblind$feature=="WIND SLIGHT")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("WIND")
  a <- which(dom_agg_cl_woon_colblind$feature=="MORNING CHORUS")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("BIRDS")
  a <- which(dom_agg_cl_woon_colblind$feature=="WIND VERY STRONG")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("WIND")
  a <- which(dom_agg_cl_woon_colblind$feature=="BIRDS LOUD")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("BIRDS")
  a <- which(dom_agg_cl_woon_colblind$feature=="RAIN HEAVY")
  dom_agg_cl_woon_colblind$feature[a] <- as.character("RAIN")
  dom_agg_cl_woon <- dom_agg_cl_woon_colblind
}
cum_date <- cumsum(days_per_month)+1
cum_date <- cum_date[1:(length(cum_date)-1)]
labels <- unique(dates)[(cum_date)]
for(i in 1:length(labels)) {
  if(substr(labels[i],5,6)=="01") {labels[i] <- paste("JAN", substr(labels[i],3,4), sep=" ")}
  if(substr(labels[i],5,6)=="02") {labels[i] <- paste("FEB", substr(labels[i],3,4), sep=" ")}
  if(substr(labels[i],5,6)=="03") {labels[i] <- paste("MAR", substr(labels[i],3,4), sep=" ")}  
  if(substr(labels[i],5,6)=="04") {labels[i] <- paste("APR", substr(labels[i],3,4), sep=" ")}  
  if(substr(labels[i],5,6)=="05") {labels[i] <- paste("MAY", substr(labels[i],3,4), sep=" ")}
  if(substr(labels[i],5,6)=="06") {labels[i] <- paste("JUN", substr(labels[i],3,4), sep=" ")}
  if(substr(labels[i],5,6)=="07") {labels[i] <- paste("JUL", substr(labels[i],3,4), sep=" ")}  
  if(substr(labels[i],5,6)=="08") {labels[i] <- paste("AUG", substr(labels[i],3,4), sep=" ")}  
  if(substr(labels[i],5,6)=="09") {labels[i] <- paste("SEP", substr(labels[i],3,4), sep=" ")}
  if(substr(labels[i],5,6)=="10") {labels[i] <- paste("OCT", substr(labels[i],3,4), sep=" ")}
  if(substr(labels[i],5,6)=="11") {labels[i] <- paste("NOV", substr(labels[i],3,4), sep=" ")}  
  if(substr(labels[i],5,6)=="12") {labels[i] <- paste("DEC", substr(labels[i],3,4), sep=" ")}  
}
...
for(i in 1:length(uniq_dates)) {
  lab <- substr(uniq_dates[i],7,8)
  labels1 <- c(labels1, lab)
}

shape <- "polygon" # Choices "circle" and "polygon"
if(shape=="circle") {
  library(plotrix)
}
...
png(paste("plots/dom_agg_gym_", version,"_",shape, 
          ".png", sep = ""), width = 5000, height = 5000)
par(mar=c(5, 5, 1, 1), xpd=FALSE, oma=c(3,3,0,0))
...
for(i in 1:length(feature_colours$Feature)) {
  ...
  for(j in a) {
    for(k in a) {
      ...
      if(shape=="circle") {
        points(k,j, pch=46, col = col_code, cex=0.3)
      }
    }
  }
}
at <- seq(0, (nrow(dom_agg_cl_gym)-1), 12)
# Add first of the month dates on both axes
axis(side = 1, line = -8.8, at = at[cum_date+2], las=1, 
     labels= labels, cex.axis = 5, cex.lab = 5, tick = F, 
     lwd = 0)
axis(side = 2, line = -10, at = at[cum_date+2], las=1, 
     labels= labels, cex.axis = 5, cex.lab = 5, tick = F,
     lwd = 0)
# Add ticks on both axes
axis(side = 1, line = -12.5, at = at, las=1, 
     labels= labels1, cex.axis = 0.8, cex.lab = 0.8, tick = T)
axis(side = 2, line = -12.5, at = (at + 0.5), las=1, 
     labels= labels1, cex.axis = 0.8, cex.lab = 0.8, tick = T)
# Add vertical and horizontal lines
par(xpd=FALSE)
abline(v = at[cum_date], lty = 2, lwd = 0.8)
abline(h = (at[cum_date] + 0.5), lty = 2, lwd = 0.8)
dev.off()

# Using dom_agg_cl_woon and feature_colours
# This plot takes 3-4 minutes to generate
png(paste("plots/dom_agg_woon_", version,"_",shape, 
          ".png", sep = ""), width = 5000, height = 5000)
par(mar=c(5, 5, 1, 1), xpd=FALSE, oma=c(3,3,0,0))
# Plot an empty plot with no axes or frame
...
for (k in 1:2) {
  ...
  axis(1, tick = FALSE, at = at, 
       labels = c("12 am","2 am","4 am",
                  ...
                  "12","2 pm","4 pm","6 pm",
                  "8 pm","10 pm","12 pm"), 
       cex.axis=1.8, line = -2.6)
  # plot the left axes
  axis(side = 2, at = first_of_each_month, tick = FALSE, 
       labels=format(dates[first_of_month],"%b %Y"), 
       cex.axis=1.6, las=1, line = -5)
  axis(side = 2, at = c(days), tick = FALSE, 
       labels=format(dates[1],"%d %b %Y"), 
       cex.axis=1.6, las=1, line = -5)
  # plot the left axes
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Description:  Plots clusters in each minute 
#               period across days
# 1. As individual clusters 
# 2. As agglomerated clusters ie grouping all clusters 
# classified as the same feature together
