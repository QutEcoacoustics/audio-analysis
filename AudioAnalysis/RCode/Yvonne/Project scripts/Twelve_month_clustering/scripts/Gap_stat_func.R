...
clusGap_par <- function (x, FUNcluster, K.max, B = 100, verbose = interactive(),
                     do_parallel = FALSE, ...)
{
  ...
  if (do_parallel)
  {
    apply_fun <- parallelsugar::mclapply
  }
  ...
  if(verbose) cat("done\n")
  print("Start Scale x into hypercube")
  ...
  logWksList <- apply_fun(1:B,
                          function(b)
                          {
                            ...
                            curLogWks <- unlist(lapply(seq(5, K.max, by = 5), function(k) log(W.k(z, k))))
