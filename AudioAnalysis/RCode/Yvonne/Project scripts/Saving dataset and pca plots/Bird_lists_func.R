...
species_plots <- function(species, col) {
  ...
  plot(x = civ_dawn+25, y = -1:-length(civ_dawn), type = "l", 
       lty=5, ylim = ylim, ylab="", xaxt="n", lwd=3,
       yaxt="n", xlim = xlim)
  at <- c(0, 60, 120, 180, 240, 300, 360, 420, 480, 540, 600)
  labels1 <- c("12 am", "1 am", "2 am", "3 am", "4 am","5 am","6 am","7 am",
               "8 am","9 am","10 am")
  abline(v=at)
...
species_plots2 <- function(label, species, col, ref) {
  par(mar=c(2,3,0.5,1), bty="n", cex=4)
  plot(x = civ_dawn, y = (ylim1[2]:ylim1[1]), type="n", axes=FALSE, ann=FALSE)
  bird_clust_plot(kalscpe_data)
  bird_plot(species, col, 20)
  legend(x=(xlim[1]-1.5),y=(ylim[2] + 1 - (ref - 1) * 2),legend=species,
         col=col, pch = 20, bty = "n", cex = 1.4)
  text(x = -10, y = xlim[1]+20, paste(pch = 4, pch = 4, pch = 4, pch = 4))
  par(new=TRUE)
  plot(x = civ_dawn, y = (ylim1[2]:ylim1[1]), 
       type = "l", yaxt="n", xaxt="n", lty = 2, 
       ylab = "", ylim = ylim, xlim = xlim, lwd = 3)
  ...
  labels <- c("Jun", "Jul", "Aug", "Sep", "Oct", "Nov",
              "Dec", "Jan", "Feb", "Mar", "Apr",
              "May", "Jun", "Jul")
  axis(side = 2, at = -a-14, line = NA, labels = labels, 
       tick = F, cex.axis = 1.4, cex.lab = 1.4)
  par(new=TRUE)
  plot(x = civ_dawn-5, y = -1:-length(civ_dawn), type = "l", 
       lty = 4, ylim = ylim, ylab="", yaxt="n",
       xaxt="n", lwd=3, xlim = xlim)
  par(new=T)
  plot(x = civ_dawn+5, y = -1:-length(civ_dawn), type = "l", 
       lty = 3, ylim = ylim, ylab="", xaxt="n", lwd=3,
       yaxt="n", xlim = xlim)
  par(new=T)
  plot(x = civ_dawn-15, y = -1:-length(civ_dawn), 
       type = "l", lty=3, ylim = ylim, ylab="", lwd=3, 
       xaxt="n", yaxt="n", xlim = xlim)
  par(new=T)
  plot(x = civ_dawn+15, y = -1:-length(civ_dawn), type = "l", 
       lty=3, ylim = ylim, ylab="", xaxt="n", lwd=3,
  ...
  abline(v=at)
  axis(side=1, at = at, labels = labels1, las=1,
       cex.axis = 1.4, line = -0.5, cex.lab = 1.4)
  if(ylim[2]==0) {
    text(x = (civ_dawn[-ylim[2]+1] - 20), y = ylim[2]+0.4, "A", cex = 1.4)
    text(x = civ_dawn[-ylim[2]+1], y = ylim[2]+0.4, "B", cex = 1.4)
    text(x = civ_dawn[-ylim[2]+1] + 20, y = ylim[2]+0.4, "C", cex = 1.4)
  }
  ...
  mtext(side = 3, label, cex = 6, line = -0.9)
}
