...
for (i in 1:7){
  for (j in 1:7){
    scoring_matrix[i,j] = as.integer((log(statetransprob[i,j]/(Pcombcd[i] * Pcombcd[j])))*4)
...
write.table(scoring_matrix, file="/Volumes/Nifty/QUT/HMM/hiddenstates/scoringmatrix.txt", row.names=TRUE, col.names=TRUE)
