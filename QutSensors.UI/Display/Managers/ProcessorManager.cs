...
namespace QutSensors.UI.Display.Managers
{
    ...
    public static class ProcessorManager
    {
        ...
        public static IEnumerable<JobResult> GetJobResults(int jobId, Expression<Func<Processor_Result, bool>> filter, int maxItems, int startIndex, string sortExpression)
        {
            ...
            {
                var results = AnalysisJobManager.Instance.GetJobResults(db, jobId, (i) => true, filter);
                return GetJobResults(db, results, jobId, maxItems, startIndex, sortExpression).ToList();
            }
        }
        ...
        public static int CountJobResults(int jobId, Expression<Func<Processor_Result, bool>> filter)
        {
            using (var db = new QutSensorsDb())
            {
                var query = AnalysisJobManager.Instance.GetJobResults(db, jobId, (i) => true, filter);
        ...
        public static IEnumerable<JobResult> GetJobResults(int jobId, Guid audioReadingId, int maxItems, int startIndex, string sortExpression)
        {
            using (var db = new QutSensorsDb())
            {
                var query = AnalysisJobManager.Instance.GetJobResults(db, jobId, null, null)
                    .Where(a => a.Processor_JobItem.AudioReadingID == audioReadingId);
            ...
            using (var db = new QutSensorsDb())
            {
                ...
                switch (sortExpression)
                {
                    ...
                    case "AudioReadingTotalLength":
                        results = results.OrderBy(r => r.AudioReading.Length);
                        break;
                    case "AudioReadingTotalLength DESC":
                        results = results.OrderByDescending(r => r.AudioReading.Length);
                        break;
                        ...
                        new JobWorkItem
                            {
                                ...
                                AudioReadingTotalLength =
                                    ji.A.Length.HasValue
                                        ? new TimeSpan(0, 0, 0, 0, ji.A.Length.Value).ToReadableString()
                                        : "unknown",
                                EndTime =
                                ...
                                AudioReadingSegmentLength =
                                    !ji.A.Length.HasValue
                                        ...
                                        : new TimeSpan(0, 0, 0, 0, (ji.J.EndTime.HasValue ? ji.J.EndTime.Value :
                                            ji.A.Length.Value) - (ji.J.StartTime.HasValue ? ji.J.StartTime.Value : 0)).ToReadableString(),
                                FilterQs =
                                      ...
                                      new
                                          {
                                              ...
                                              ReadingLength = jiAr.Key.Length,
                                              HardwareName = jiAr.Key.Hardware.FriendlyName,
                                              JobResultCount =
                                      db.Processor_Results.Where(
                                          procr =>
                                          procr.Processor_JobItem.AudioReadingID == jiAr.Key.AudioReadingID &&
